...
def create_graph(workspace, name, node_tables, edge_table):
    return multinet_request(
        f"""mutation {{
        graph(workspace: "{workspace}", name: "{name}", node_tables: \
            {json.dumps(node_tables)}, edge_table: "{edge_table}") {{
            name
        }}
    }}"""
    )
...
def table_rows(workspace, name, key=None, search=None):
    key_arg = f'key: "{key}"' if key else ""
    search_arg = f'search: "{search}"' if search else ""
    ...
    return multinet_request(
        f"""query {{
        table(workspace: "{workspace}", name: "{name}") {{
            rows{all_args} {{
                total
                data(offset: 0, limit: 10) {{
                    key
